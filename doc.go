...
Package rocksgo provides the ability to create and access rocksdb databases.

rocksgo.Open opens and creates databases.

	opts := rocksgo.NewOptions()
	opts.SetCache(rocksgo.NewLRUCache(3<<30))
	opts.SetCreateIfMissing(true)
	db, err := rocksgo.Open("/path/to/db", opts)
...
your database.
	...
	filter := rocksgo.NewBloomFilter(10)
